...
export async function registerCommands(client, guildId) {
    try {
        ...
        // Validatie van alle commando's
        logger.info('Validating commands before registration...');
        let validationErrors = [];
        commands.forEach((cmd) => {
            if (cmd.name && cmd.name.length > 32) {
                validationErrors.push(`Command ${cmd.name} has name longer than 32 chars: "${cmd.name}"`);
            }
            if (cmd.description && cmd.description.length > 110) {
                validationErrors.push(`Command ${cmd.name} has description longer than 110 chars: "${cmd.description}"`);
            }
        });
        ...
        if (guildId) {
            // REGISTRATIE VOOR EÉN SPECIFIEKE SERVER
            ...
            // GLOBALE REGISTRATIE VOOR ÁLLE SERVERS!
            logger.info(`Preparing to register ${commandsToRegister.length} GLOBAL commands for ALL guilds...`);
